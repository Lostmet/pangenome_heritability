...
def align_group(threads: str, group_name: str, sequences: List[str], temp_dir: Path, log_dir: Path, has_insertion: bool, poly_ins_list) -> AlignmentResult:
    ...
    if has_insertion:
        ...
        for i, ins_start_end in enumerate(poly_ins_list):
            input_fasta_sliced = temp_dir / f"{group_name}_input_sliced_{i+1}.fasta"
            output_fasta_sliced = temp_dir / f"{group_name}_aligned_sliced_{i+1}.fasta"
            #print(ins_start_end)
            with open(input_fasta_sliced, "w") as f:
                for i, seq in enumerate(sequences):
                    f.write(f">seq{i}\n{seq[ins_start_end['start']:ins_start_end['end']]}\n") #进行切片
            run_mafft(threads, input_fasta_sliced, output_fasta_sliced, log_dir=log_dir)#切片部分进行align操作
            sliced_fasta = parse_fasta(output_fasta_sliced)
            #print(f"切片：{sliced_fasta}")
            try:
                for key in origin_fasta: # {seq0:xxxxxxx, seq1:xxxxxxx}
                    #print(f"seq_id:{key}")
                    if key in sliced_fasta:
                        sliced_list = list(sliced_fasta[key])
                        ori_list = list(origin_fasta[key])
                        ori_list[ins_start_end['start']:ins_start_end['end']] = sliced_list[:]

                        origin_fasta[key] = "".join(ori_list)
            except:
                click.echo(f"Warning: Cannot replace sliced align results in {group_name}") 
